...
const Sherlock: React.FC = () => {
    const [username, setUsername] = useState<string>(""); 
    const [file, setFile] = useState<File | null>(null);
    const [jobType, setJobType] = useState<string>("sherlock");
    const [message, setMessage] = useState<string>("");
    const router = useRouter();

    const handleSubmit = () => {
        scheduleJob("sherlock", { input: username }).then((res) => {
            console.log(res);
        })
    }
        ...
        <div className="bg-backgroundImage min-h-screen">
            <AppBar>
                <div className="flex gap-4">
                    ...
                    <h1 className="text-xl font-bold">Sherlock</h1>
                </div>
            </AppBar>
